...
def tune_stacking_optuna(X, y, numeric_features, categorical_features, data_dir, n_trials=5):
    """
    ...
    """
    db_path = os.path.abspath(os.path.join(data_dir, "optuna_study_telecom_standalone.db"))
    storage_url = f"sqlite:///{db_path}"
    study_name = "optuna_study_telecom_standalone"
...
def train_and_evaluate():
    ...
    # Since the datasets generated by run_pipeline.py are already One-Hot Encoded, 
    # we treat all variables as numeric columns (categorical_features = []) for the DataFrameCaster.
    numeric_features = X_train_raw.columns.tolist()
    ...
    # 1. Hyperparameter Tuning for base estimators (Unified Optuna study with resilient SQLite database storage)
    logger.info("\n=== Unified Stacking Hyperparameter Tuning (Optuna) ===")
    best_params = tune_stacking_optuna(X_train_fit, y_train_fit, numeric_features, categorical_features, data_dir, n_trials=5)
    ...
    # Wrap base estimators in pipelines
    xgb_pipe = Pipeline([
        ('caster', DataFrameCaster(numeric_features, categorical_features, to_string=True)),
        ('ohe', ColumnTransformer([('ohe', OneHotEncoder(sparse_output=False, handle_unknown='ignore'), categorical_features)], remainder='passthrough')),
        ('xgb', xgb_best)
    ...
    # Quality Guardrail Check (deviation < 5%)
    logger.info("Verifying empirical conformal coverage guardrails...")
    _, y_pis_cal = mapie_model.predict_set(X_calib)
    for idx, target_lvl in enumerate(confidence_levels):
        empirical_cov = np.mean(y_pis_cal[np.arange(len(y_calib)), y_calib, idx])
        ...
        if deviation > 0.05:
            raise CalibrationQualityException(
                f"Conformal calibration quality check failed in model.py. "
